...
export class CompletedOrdersComponent implements OnInit{
  ...
  orders : any;
  displayedColumns: string[] = ['No', 'Id', 'UserName', 'OrderTime', 'PaymentMethod', 'OrderAddress', 'Total', 'Status', 'Detail'];
  index = 1;
  user : any;
  keyword: any;
  p: any;
      ...
      .pipe(
          switchMap(res => {
            ...
            if(this.user.profile.type === 'admin'){
              return this.httpClient.get(environments.api+'/api/products/admin/orders/completed/cancelled')
            }else{
              return EMPTY;
            }
          })
  ...
  search(){
    ...
    if(this.keyword === undefined || this.keyword.trim() === ''){
      ...
      this.httpClient.get(environments.api+`/api/products/admin/order/completed/cancelled/search/${this.keyword}`)
        .subscribe(res => {
          if(Array.isArray(res)){
            res.forEach((item:any) => {
  ...
  showDetail(orders : any){
    ...
    this.router.navigate([`admin/orders/completed/${orders.id}`]).catch(error => {console.log(error)});
  }
